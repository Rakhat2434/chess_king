...
const statusOptions = [
  { value: 'new', labelKey: 'admin.statusNew', color: 'bg-blue-100 text-blue-700' },
  { value: 'processing', labelKey: 'admin.statusProcessing', color: 'bg-yellow-100 text-yellow-700' },
  { value: 'confirmed', labelKey: 'admin.statusConfirmed', color: 'bg-green-100 text-green-700' },
  { value: 'cancelled', labelKey: 'admin.statusCancelled', color: 'bg-red-100 text-red-700' },
];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-king-navy">{t('admin.enrollmentsTitle')}</h1>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {item.age && <span className="text-xs text-king-gray bg-gray-100 px-2 py-0.5 rounded-full">{t('admin.ageYears', { age: item.age })}</span>}
                  </div>
                  ...
                  {item.coach && <p className="text-xs text-king-gray mt-1">{t('admin.coachLabel', { name: item.coach.name })}</p>}
                  {item.comment && <p className="text-xs text-king-gray mt-1 italic">&quot;{item.comment}&quot;</p>}
                  <p className="text-xs text-gray-400 mt-2"><LocalizedDate value={item.createdAt} /></p>
                </div>
